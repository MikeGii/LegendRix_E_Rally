...
// Rally interface that matches Supabase schema
interface Rally {
  id: string
  name: string
  game: string
...
// Transform for RallyDisplay component compatibility
interface TransformedRally {
  rally_id: string
...
export function UserDashboard() {
  ...
  const [upcomingRallies, setUpcomingRallies] = useState<TransformedRally[]>([])
  ...
  const loadUpcomingRallies = async () => {
    try {
      ...
      console.log('Loading upcoming rallies from Supabase...')
      ...
      // Transform data to match RallyDisplay component expectations
      const transformedRallies: TransformedRally[] = rallies?.map(rally => ({
        ...
        events: [], // Will be populated when events system is added
        creator_name: 'Rally Admin'
      })) || []

      setUpcomingRallies(transformedRallies)
