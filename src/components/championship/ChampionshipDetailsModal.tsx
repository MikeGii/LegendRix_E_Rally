...
import {
  useChampionshipResults,
  ChampionshipParticipant,
} from "@/hooks/useChampionshipResults";
import {
...
export function ChampionshipDetailsModal({
  ...
  const {
    ...
    error,
    refetch,
  } = useChampionshipResults(championshipId);
  ...
  const handleCompleteChampionship = async () => {
    if (
      window.confirm(
        `Kas olete kindel, et soovite meistrivõistluse "${results.championship_name}" lõpetatuks märkida?`
      )
    ) {
      try {
  ...
  const handleReopenChampionship = async () => {
    if (
      window.confirm(
        `Kas olete kindel, et soovite meistrivõistluse "${results.championship_name}" uuesti avada?`
      )
    ) {
      try {
  ...
  const sortedRallies =
    results.participants?.[0]?.rally_scores
      ?.map((rs) => ({
  ...
  const filteredParticipants =
    selectedClass === "all"
      ...
      : (results.participants || []).filter(
          (p) => p.class_name === selectedClass
        );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div className="bg-slate-800 rounded-xl p-6">
          ...
          <button
            onClick={onClose}
            className="mt-4 px-4 py-2 bg-slate-600 rounded"
          >
            Sulge
          </button>
        </div>
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-700">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 text-sm text-slate-400">
                  ...
                  <span>
                    📝 {results.unlinked_participants} käsitsi lisatud
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              {!showCompleted ? (
                ...
                >
                  {completeChampionshipMutation.isPending
                    ? "Lõpetan..."
                    : "🏁 Märgi lõpetatuks"}
                </button>
              ) : (
                ...
                >
                  {reopenChampionshipMutation.isPending
                    ? "Avan..."
                    : "🔄 Ava uuesti"}
                </button>
              )}
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex space-x-2">
                ...
                {availableClasses.map((className) => {
                  const classCount = (results.participants || []).filter(
                    (p) => p.class_name === className
                  ).length;
                  return (
                    <button
                      key={className}
                  ...
                  <thead className="sticky top-0 bg-slate-800/90 backdrop-blur">
                    <tr className="border-b border-slate-700">
                      ...
                      {sortedRallies.map((rally) => (
                        <th
                          key={rally.rally_id}
                          className="px-2 py-3 text-center font-medium text-slate-300 min-w-[80px]"
                        >
                      ...
                      if (selectedClass === "all") {
                        ...
                        Array.from(groupedByClass.entries())
                          ...
                          .forEach(([className, classParticipants]) => {
                            ...
                            const sortedParticipants = classParticipants.sort(
                              (a, b) => {
                                const posA = a.championship_position || 999;
                                const posB = b.championship_position || 999;
                                if (posA !== posB) return posA - posB;
                                if (
                                  a.total_overall_points !==
                                  b.total_overall_points
                                )
                                  return (
                                    b.total_overall_points -
                                    a.total_overall_points
                                ...
                                return (
                                  b.rounds_participated - a.rounds_participated
                                );
                              }
                            );
                            ...
                            sortedParticipants.forEach((participant, index) => {
                              ...
                              rows.push(
                                ...
                                >
                                  ...
                                  {sortedRallies.map((rally) => {
                                    const rallyScore =
                                      participant.rally_scores.find(
                                        (rs) => rs.rally_id === rally.rally_id
                                      );
                                    return (
                                      <td
                                        key={rally.rally_id}
                        ...
                        const sortedParticipants = filteredParticipants.sort(
                          (a, b) => {
                            ...
                            if (posA !== posB) return posA - posB;
                            if (
                              a.total_overall_points !== b.total_overall_points
                            )
                              return (
                                b.total_overall_points - a.total_overall_points
                              );
                            return (
                              b.rounds_participated - a.rounds_participated
                            );
