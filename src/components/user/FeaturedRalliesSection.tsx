// src/components/user/FeaturedRalliesSection.tsx - Updated version
...
export function FeaturedRalliesSection({ rallies, isLoading, canAccessRallies, onRegister }: FeaturedRalliesSectionProps) {
...
// Simple Rally Display Component (fallback for compatibility)
interface SimpleRallyDisplayProps {
  rallies: RealRally[]
  showLimit?: number
...
export function SimpleRallyDisplay({ rallies, showLimit, showRegistration = false, onRegister }: SimpleRallyDisplayProps) {
  ...
  const isRegistrationOpen = (rally: RealRally) => {
    ...
    return rally.status === 'registration_open' && deadline > now
  }
