...
export function AdminDashboard() {
  ...
  const fetchPendingUsers = useCallback(async (forceRefresh = false) => {
    try {
      ...
      console.log(`✅ Fresh data received: ${users?.length || 0} users`)
      ...
      const stats = {
        ...
        pendingEmail: transformedUsers.filter(u => u.status === 'pending_email').length,
        pendingApproval: transformedUsers.filter(u => 
          u.status === 'pending_approval' && 
          u.email_verified && 
          !u.admin_approved
        ).length,
        approved: transformedUsers.filter(u => u.status === 'approved').length,
        rejected: transformedUsers.filter(u => u.status === 'rejected').length
      }
